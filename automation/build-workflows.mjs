...
const output={name:'BetterCallz — Completed Calls → Sales Log',active:false,nodes:[
 ...
 node('Each result','code',{jsCode:`return ($input.first().json.results || []).map(r=>{if(!Number.isSafeInteger(r.sheet_row)||r.sheet_row<2||r.sheet_row>10000||!Array.isArray(r.sheet_payload)||r.sheet_payload.length!==26)throw new Error('Invalid output or sales log capacity reached');return {json:r};});`},[500,0]),
 node('Check existing row','httpRequest',{authentication:'predefinedCredentialType',nodeCredentialType:'googleSheetsOAuth2Api',url:`={{ 'https://sheets.googleapis.com/v4/spreadsheets/${sheetId}/values/Leads!A' + $json.sheet_row + ':Z' + $json.sheet_row }}`,options:{timeout:30000}},[750,0],{typeVersion:4.2,retryOnFail:true,maxTries:3}),
 node('Protect previous leads','code',{mode:'runOnceForEachItem',jsCode:`const result=$('Each result').item.json;const existing=$json.values?.[0]||[];if(existing.some(v=>String(v)!=='') && (existing[2]!==result.meta_lead_id || existing[20]!==result.call_id))throw new Error('Allocated row belongs to another record. Restore row order; refusing to overwrite a lead.');return {json:result};`},[1000,0]),
 node('Write fixed sales row','httpRequest',{authentication:'predefinedCredentialType',nodeCredentialType:'googleSheetsOAuth2Api',method:'PUT',url:`={{ 'https://sheets.googleapis.com/v4/spreadsheets/${sheetId}/values/Leads!A' + $json.sheet_row + ':Z' + $json.sheet_row + '?valueInputOption=RAW' }}`,sendBody:true,specifyBody:'json',jsonBody:'={{ {values: [$json.sheet_payload]} }}',options:{timeout:30000}},[1250,0],{typeVersion:4.2,retryOnFail:true,maxTries:3}),
 ...
 http('Acknowledge delivered row',{method:'POST',url:'https://demo.bettercallz.com/api/meta-leads/results',sendBody:true,specifyBody:'json',jsonBody:'={{ {call_id:$json.call_id,sheet_row:$json.sheet_row} }}'},[1750,0]),
